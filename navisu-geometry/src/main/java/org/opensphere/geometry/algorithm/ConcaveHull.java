 ...
 */
/*
Doc : http://www.rotefabrik.free.fr/concave_hull/
...
public class ConcaveHull {
    ...
    private Geometry concaveHull() {
        ...
        Collection<QuadEdge> quadEdges = qes.getEdges();   //qes.getEdges();
        ...
        quadEdges.stream().map((qe) -> {
            ...
            return qe;
        }).filter((qe) -> (qes.isFrameEdge(qe))).forEach((qe) -> {
            qeFrame.add(qe);
        });
        ...
        qeFrame.stream().forEach((qe) -> {
            qes.delete(qe);
        });

        HashMap<QuadEdge, Double> qeDistances = new HashMap<>();
        quadEdges.stream().forEach((qe) -> {
        ...
        this.edges.values().stream().filter((edge) -> (edge.getTriangles().size() != 1)).forEach((edge) -> {
        ...
        this.lengths.values().stream().map((e) -> e.getGeometry().toGeometry(this.geomFactory)).forEach((l) -> {
            edges.add(l);
        });

        this.shortLengths.values().stream().map((e) -> e.getGeometry().toGeometry(this.geomFactory)).forEach((l) -> {
            edges.add(l);
        });
