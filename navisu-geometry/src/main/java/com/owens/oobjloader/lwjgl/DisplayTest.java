...
public class DisplayTest {
        ...
        for (Face face : builder.faces) {
            if (face.material != currentMaterial) {
                if (!currentFaceList.isEmpty()) {
                    log.log(INFO, "Adding list of {0} triangle faces with material {1}  to our list of lists of faces.", new Object[]{currentFaceList.size(), currentMaterial});
                    facesByTextureList.add(currentFaceList);
                }
                log.log(INFO, "Creating new list of faces for material {0}", face.material);
    ...
    private static void run(String filename, String defaultTextureMaterial) {
        ...
        log.log(INFO, "Done splitting OBJ file faces into list of faces per material, ended up with {0} lists of faces.", facesByTextureList.size());
        System.out.println("facesByTextureList : " + facesByTextureList);
      ...
      /*  
        ...
        if (defaultTextureMaterial != null) {
            log.log(INFO, "Loading default texture ={0}", defaultTextureMaterial);
            defaultTextureID = setUpDefaultTexture(textureLoader, defaultTextureMaterial);
            log.log(INFO, "Done loading default texture ={0}", defaultTextureMaterial);
        }
