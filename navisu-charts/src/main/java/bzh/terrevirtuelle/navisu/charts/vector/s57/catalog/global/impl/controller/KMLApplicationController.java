 ...
 * This controller animates the view to a KML feature when the feature is
 * clicked in the feature tree, and animates the view to KML network links when
 * they are refreshed.
 * <p/>
 * This controller may optionally be associated with a
 * {@link BalloonController}. If a BalloonController is set, this controller
 * will open the description balloon for a KML feature when the feature is
 * clicked in the feature tree.
 *
 * @author pabercrombie
 * @version $Id: KMLApplicationController.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class KMLApplicationController
        implements SelectListener, PropertyChangeListener {

    /**
     * Indicates the <code>WorldWindow</code> this controller listens to for
     * select events and property change events. Initialized during
     * construction.
     */
    protected WorldWindow wwd;
    /**
     * Indicates the <code>TreeNode</code> currently under the cursor. Initially
     * <code>null</code>.
     */
 ...
 //   protected BalloonController balloonController;
    ...
    /**
     * Creates a new <code>KMLApplicationController</code> with the specified
     * <code>WorldWindow</code>. The new <code>KMLApplicationController</code>
     * listens KML tree node select events, and <code>KMLNetworkLink</code>
     * refresh property change events.
     *
     * @param wwd the <code>WorldWindow</code> this listens to, and who's
     * <code>View</code> is moved upon a refresh event.
     *
     * @throws IllegalArgumentException if the <code>wwd</code> is
     * <code>null</code>.
     */
    ...
    /**
     * Indicates the BalloonController associated with this controller. The
     * BalloonController is used to open balloons for KML features when the
     * feature is selected.
     *
     * @return Active BalloonController. May return {@code null} if no
     * BalloonController is set.
     */
    /*
...
*/
    /**
     * Specifies a BalloonController that this controller can use to open
     * balloons when KML features are selected.
     *
     * @param balloonController New BalloonController. May be {@code null} to
     * unset the balloon controller.
     */
    /*
...
*/
    /**
     * Animate the globe to a KML feature when the feature is clicked in the
     * tree.
     *
    ...
    /**
     * Handles property change events sent from <code>KMLNetworkLink</code>
     * objects to the <code>SceneController</code>. Upon receiving a
     * <code>{@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}</code>
     * event from a <code>KMLNetworkLink</code>, this attempts to fly to a
     * <code>KMLAbstractView</code> associated with the link's KML resource.
     * <p/>
     * If the <code>KMLNetworkLink</code>'s <code>flyToView</code> property is
     * <code>0</code> or <code>false</code>, this ignores the event. Otherwise,
     * this attempts to get a <code>KMLAbstractView</code> from features in the
     * link's KML resource as follows: <ol> <li><code>NetworkLinkControl</code>
     * child of link's KML resource.</li>
     * <li><code>AbstractFeature</code> child of link's KML resource.</li> </ol>
    ...
    /**
     * Called from <code>propertyChange</code> when a
     * <code>KMLNetworkLink</code> sends a <code>{@link
     * gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}</code>
     * property change event. This attempts to fly to a view associated with the
     * link's KML resource.
     * <p/>
     * This does nothing if the <code>networkLink</code> is <code>null</code>.
     *
     * @param networkLink the <code>KMLNetworkLink</code> that has been
     * refreshed.
     */
    ...
    /**
     * Invoked when a feature is selected in the KML feature tree. This
     * implementation animates the view to look at the feature, and opens the
     * feature's balloon.
     *
...
*/
    /**
     * Smoothly moves the <code>WorldWindow</code>'s <code>View</code> to the
     * specified <code>KMLAbstractFeature</code>.
     *
    ...
    /**
     * Smoothly moves the <code>WorldWindow</code>'s <code>View</code> to the
     * specified <code>KMLAbstractView</code>.
     *
    ...
    /**
     * Determines if there is a some action (fly to and/or open description
     * balloon) to take when the user selects a KML feature in the tree. The
     * controller displays a hand cursor when the mouse is over a feature if the
     * feature can be flown to, or if the has a balloon that can be opened.
     *
     * @param feature KML feature to test.
     *
     * @return {@code true} if the controller can either animate the view or
     * open a balloon for the feature.
     */
     ...
     * @param feature KML feature to test.
     *
     * @return {@code true} if the feature has a view associated with it. This
     * can be an explicit <i>Camera</i> or
     * <i>LookAt</i>, or a default view for a geographic feature. Not all
     * features have a default view. For example, features attached to screen
     * (<i>ScreenOverlay</i>) and container features (<i>Document</i> and
