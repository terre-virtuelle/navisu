 ...
 */
public class AnalyticSurfaceController_old {
    ...
    public AnalyticSurfaceController_old(RenderableLayer layer,
            double[] data,
    ...
    final void createSurface(double minHue, double maxHue,
        ...
        BufferWrapper firstBuffer = AnalyticSurfaceController_old.this.gridValues(width, height, minValue, maxValue);
        BufferWrapper secondBuffer = AnalyticSurfaceController_old.this.gridValues(width, height, minValue, maxValue);
    ...
    protected static void mixValuesOverTime(
        ...
        surface.setValues(createMixedColorGradientGridValues(a, firstBuffer, secondBuffer, minValue, maxValue, minHue, maxHue));
    ...
    public BufferWrapper gridValues(
        ...
        int numValues = vectorField.length;

        smoothValues(width, height, vectorField, smoothness);
        scaleValues(vectorField, numValues, min, max);
        BufferWrapper buffer = factory.newBuffer(numValues);
        buffer.putDouble(0, vectorField, 0, numValues);

        return buffer;
    }

    public BufferWrapper gridValues(int width, int height, double min, double max) {
