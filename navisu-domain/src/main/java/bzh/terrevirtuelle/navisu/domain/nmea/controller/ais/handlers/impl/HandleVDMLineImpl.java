package bzh.terrevirtuelle.navisu.domain.nmea.controller.ais.handlers.impl;

import bzh.terrevirtuelle.navisu.domain.nmea.controller.ais.handlers.HandleInvalidVDMLine;
import bzh.terrevirtuelle.navisu.domain.nmea.controller.ais.handlers.HandleVDMLine;
import bzh.terrevirtuelle.navisu.domain.nmea.controller.ais.handlers.HandleVDMMessage;
import bzh.terrevirtuelle.navisu.domain.nmea.model.ais.impl.VDMLine;
import bzh.terrevirtuelle.navisu.domain.nmea.model.ais.impl.VDMMessage;

public class HandleVDMLineImpl implements HandleVDMLine {

    private final HandleVDMMessage handler;
    private final HandleInvalidVDMLine errorHandler;

    public HandleVDMLineImpl(HandleVDMMessage handler, HandleInvalidVDMLine errorHandler) {
        this.handler = handler;
        this.errorHandler = errorHandler;
    }

    private VDMMessage current = null;

    @Override
    public void handleVDMLine(VDMLine line) {
        if (line.isSemanticallyValid()) {
            if (current == null) {
                current = new VDMMessage(line);
            } else {
                if (current.isPartOfMessage(line)) {
                    current.add(line);
                } else {
                    handler.handleVDMMessage(current);	//correctness must be checked by VDMMessageHandler	 
                    current = new VDMMessage(line);
                }
            }
            //When message is complete, we don't wait for the next message before we process it.
            if (current.atEnd()) {
                handler.handleVDMMessage(current);
                current = null;
            }
        } else {
            errorHandler.handleInvalidVDMLine(line);
        }

    }

    @Override
    public void finished() {
        if (current != null) {
            handler.handleVDMMessage(current);
            //TODO: must finished message be passed on even further?
        }
    }
}
