...
/**
 * This class provides functions to analyze the estimated-time-of-arrival (ETA)
 * value that is returned by {@link AISMessage05#getEta()}.
 *
...
public class UtilsEta {
     ...
     *
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
     * @return the minute of the ETA (0-59) <br> 60 =
     * {@link #UNAVAILABLE_MINUTE} = unavailable
     */
     ...
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
     * @return the hour of the ETA (0-23) <br> 24 = {@link #UNAVAILABLE_HOUR} =
     * unavailable
     */
     ...
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
     * @return the day of the ETA (1-31) <br> 0 = {@link #UNAVAILABLE_DAY} =
     * unavailable
     */
     ...
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
     * @return the month of the ETA (1-12) <br> 0 = {@link #UNAVAILABLE_MONTH} =
     * unavailable
     */
    ...
    /**
     * Checks whether the estimated-time-of-arrival (ETA) {@code BitVector} is
     * semantically correct (according to the standard). This means that every
     * field is either a valid value or the standard value indicating
     * unavailability. Note that this does not check whether the given month
     * actually has enough days to contain the given day.
     *
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
    ...
    /**
     * Checks whether the estimated-time-of-arrival (ETA) is available. This
     * means that at least one of the components is available.
     *
     * @param eta a <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
    ...
    /**
     * Calculates the time indicated by the ETA value. For this purpose we take
     * the earliest time after a given reference time that corresponds to the
     * ETA value.
     *
     * @param eta an ETA <code>BitVector</code> as returned by
     * {@link AISMessage05#getEta()}
    ...
    public static long convertToTime(BitVector eta, double timeNow) {
        ...
        /**
         * The number of the calendar field that will be incremented (to ensure
         * we get an ETA in the future) if we find an ETA field that is less
         * than the corresponding reference field. We will use Calendar.ERA to
         * signal that overflow is no longer necessary (since we have a future
         * time already).
         */
    ...
    /**
     * This constructor is made private to indicate that clients should not
