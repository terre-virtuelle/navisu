...
import bzh.terrevirtuelle.navisu.domain.nmea.model.ais.impl.utils.CharType;
...
public class VDMMessage {
     ...
     *
     * @param line
     * @return boolean representing whether VDM Line can be use to create a VDM
     * Message (i.e. whether it is valid VDM Line)
     */
    ...
    /**
     * get Message of VDM Message. If the Message is incomplete, the message
     * only contains the payLoad from the lines before the first missing line.
     *
    ...
    /**
     * Is VDM line part of this VDM message? The parser accepts Same number of
     * parts, higher partId, same channelId, and same messageId and Same number
     * of parts, next partId, and either same channelId or (same messageId and
     * same channelIdType)
     *
     * @param line
    ...
    public boolean isPartOfMessage(VDMLine line) {
        if ((nrOfParts == line.getNrOfParts()) && (lastPartId < line.getPartId())) {
            ...
            } else {   //lastPartId +1 < line.getPartId()
    ...
    /**
     * Add part to VDM Message Small standard violations are accepted but
     * documented using annotations as part of the provenance Note: missing
     * lines are not annotated. It is a fact, and not a violation of the
     * standard, that lines can get lost!
     *
     * @param line
     */
