...
public class ElevationLoader {
    ...
    public void compute() {
        ...
        for (double lat = positions.get(0).getLatitude().getDegrees();
            ...
            for (double lon = positions.get(0).getLongitude().getDegrees();
                    ...
                    lon += lonRange) {
                elevations += model.getElevation(Angle.fromDegrees(lat), Angle.fromDegrees(lon)) + " ";
            }
        }
    ...
    private void write() {
        String txt = //"<Transform rotation='1 0 0 3.14116'> \n"
                "<Shape>\n"
                + "<Appearance>\n"
                + "<Material/>\n"
                + "</Appearance>\n"
                + "<ElevationGrid \n"
                + "solid='false' xDimension='200'\n"
                + " xSpacing='" + latRangeMetric + "' "
                + " zDimension='200'"
